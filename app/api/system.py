...
async def get_stats(db: Session = Depends(get_db)):
    ...
    active_jobs = db.query(func.count(PollingJob.id)).filter(
        PollingJob.status == "active"
    ).scalar() or 0
    paused_jobs = db.query(func.count(PollingJob.id)).filter(
        PollingJob.status == "paused"
    ).scalar() or 0
    completed_jobs = db.query(func.count(PollingJob.id)).filter(
        PollingJob.status == "completed"
    ).scalar() or 0
    failed_jobs = db.query(func.count(PollingJob.id)).filter(
