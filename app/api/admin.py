...
async def create_secret(secret_data: SecretCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        secret = Secret(name=secret_data.name, type=secret_data.type, value=encrypted_value)
...
async def delete_secret(secret_id: int, db: Session = Depends(get_db)):
    ...
    if jobs_using_secret:
        ...
        logger.warning(f"Attempted to delete secret {secret_id} referenced by jobs: {job_ids}")
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
    ...
    return MessageResponse(message=f"Secret '{secret.name}' deleted successfully", success=True)
